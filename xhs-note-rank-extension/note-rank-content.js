...
function collectNoteRankFromDom() {
  ...
  if (!root) {
    return { ok: false, error: "未找到 note-rank 容器，请确认当前为笔记排行页面。" };
  }
 ...
 * 导出字段：排名、店铺名、粉丝数、笔记阅读数、笔记商品点击率、笔记支付转化率、笔记成交金额（元）
 */
function collectAccountRankFromDom() {
  ...
  if (!root) {
    return { ok: false, error: "未找到成交榜容器，请确认当前为成交榜页面。" };
  }
...
function ensureSidePanel() {
  ...
  panel.innerHTML = `
    ...
    <div style="margin-bottom:4px;display:flex;flex-wrap:wrap;gap:4px;">
      <button id="xhs-account-rank-btn-collect" style="flex:1 0 90px;padding:3px 6px;font-size:12px;cursor:pointer;">采集成交榜账号</button>
